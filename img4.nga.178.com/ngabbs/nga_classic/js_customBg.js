...
if(a[1]==0){
	if(!window.Trianglify)
		return loader.script(__COMMONRES_PATH+'/trianglify.min.js?'+__SCRIPTS.rand,function(){commonui.customBackgroundInit(a)})
	a[4] = Trianglify({
		height: a[3],
		width: a[2],
...
if(ubbcode.extendtop && $('extendtop'))
	ubbcode.extendtop.remove()
...
//else{
	var cok = "customBackgroundOverlay_2_"+s+"_"+__COLOR.bg0, l1="iVBORw0KGgoAAAANSUhEUgAAAAEAAAGQCAYAAABvWArbAAAAGXRFWHRTb2Z0d2FyZQBBZG9iZSBJbWFnZVJlYWR5ccllPAAAACVJREFUeNrsxjENAAAIwLAFS/jXNnSQrFdRGWBprbXW2tOdAAMAUpF/L5IYxqYAAAAASUVORK5CYII="
	/*
	if(!l1){
		if(!w.PNGlib){
			___LOADED=false
			return __SCRIPTS.asyncLoad(__COMMONRES_PATH+'/js_pnglib.js?'+__SCRIPTS.rand, function(){commonui.customBackground(u,l,os,v,opt,vbgc)})
			}
...
*/
	...
	var x = $('/div','className','overlay','style','height:100.5%;boxShadow:inset 0 0 1.5em 0 black','style',{background:'url(data:image/png;base64,'+l1+') repeat-x left top'},
...
___OJ = $('/div','_moveY',os-s,'style','height:'+os+'px;margin-bottom:-'+os+'px;overflow:hidden;background-position:'+(___TRANS&16 ? 'top' : 'bottom')+' left;background-repeat:repeat')
___OJ.style.backgroundImage='url("'+u+'")'
if(___TRANS&4)
	___OJ.$0('style','margin:-'+(os-s)+'px 0 -'+s+'px 0;transform:translateY(0px);transition:transform 45s linear')
...
if(v){
	...
	if(ts){
		...
		vo = $('/video').$0('id','videobg','muted',true,'preload','none','style','position:absolute;left:0px;top:'+z+'px;zIndex:-2;height:'+s+'px;backgroundSize:cover;transform:scale(1)',
