...
ubbcode.reset = function(){//无刷新加载时重置到初始状态
if(this.imgGen && this.imgGen.c)
	this.imgGen.c = 0
}//
...
{
return txt.replace(/</g,'&lt;').replace(/>/g,'&gt;').replace(/"/g,'&quot;').replace(/'/g,'&#39;').replace(/\$/g,'&#36;').replace(/\x5C/g,'&#92;')
//return txt.replace(/<br\/?>/g,'\x07').replace(/</g,'&lt;').replace(/>/g,'&gt;').replace(/"/g,'&quot;').replace(/'/g,'&#39;').replace(/\$/g,'&#36;').replace(/\x5C/g,'&#92;').replace(/\x07/g,'<br />')
}
//fe

ubbcode.unSecureText = function(txt,br){
if(br)
	txt = txt.replace(/\x07/ig,"\r\n").replace(/<br\s*\/?>/ig,"\r\n")
...
if(this.s!==undefined)
	return this.s
this.s = __SETTING.width ? ((__SETTING.width*0.7)|0) : 650 //默认
...
ubbcode.getAvilWidthC = commonui.getAvilWidth
ubbcode.getStyleInt = commonui.getStyleInt
ubbcode.getStyle = commonui.getStyle

ubbcode.getAvilWidth = function(p){//#获取可用宽度
var fb = 0
while(p=p.parentNode){
	if(p.nodeName=='BODY')
		return 0
	else if(p._avilWidth)
		return p._avilWidth
	else if(p.className.match(/quote|collapse_content|ubbcode|ulist|video/)){
		if(this.getStyle(p,'display').match(/block/)){
			return this.getAvilWidthC(p)
			}
		else
			fb = 1
		}
	else if(fb==1){
		if((p.nodeName=='TD') && (p.id.indexOf('postcontainer')!=-1 || p.className.indexOf('comment_c')!=-1)){
			return this.getAvilWidthC(p)
			}
		else if(this.getStyle(p,'display').match(/block/)){
			return this.getAvilWidthC(p)
			}
		}
	}
...
var arg = {
	...
	tId
	pId
	opt &1解析style &2在fixsize的折叠或随机块中(自动设置) &4fixsize限高 &8在快速阅读的弹出窗口中 &16在置顶中 &32限制图片像素(面积) &64同isBlock(自动设置) &128在签名中 &256渲染完成后发生长宽变化需调用onResize  &512显示视频  &1024忽略帖子中视频数量限制 &2048,imgGen返回array &4096限制图片像素(更多) &8192拆分表格成一列 &16384...成两列 &32768运行内嵌的jscode
	authorId &65536解析markdown
	c 要解析内容的元素
	txt 或者要解析的文字
			注意文字中遇到换行'\n'时意为bbscode分段 所有已开始的tag应被强制结束
	noImg &1显示较少/小的图 &2不显示图 &524288在&1且是附件时使用小缩略图 &1048576同前,使用中等缩略图 &2097152不显示表情
	fId
	authorId
	rvrc 无用
	isSig 是签名
	...
	isLesser 无用
	isNukePost 1是被处罚的帖子 2作者在主题中被处罚 3locked user
	isNukePostAdd 辅助
	prefix 内容的前后缀
	suffix 内容的前后缀
	...
	maxWidthO 用于计算最大宽度的元素 如果不设则使用c
	}
*/
...
if(!arg.maxWidth)//获取可用的宽度
	arg.maxWidth = this._argMaxWidth(arg)//这个是Object 不能直接if
...
//if(arg.tId>43919996 || arg.tId==0 || arg.pId>820323830)
//	arg.opt |= 65536
...
arg.txt = arg.txt.replace(/<br\s*\/?>/g,'\x07')

if((arg.opt&128)==0){
	var x = arg.txt
	if(x.indexOf('[quote]')!=-1)
		x = x.replace(/\[quote\](.+?)\[\/quote\]\x07*/gi,'')
	if(x.indexOf('Reply to [pid=')!=-1)
		x = x.replace(/^\[b\]Reply to \[pid=\d+,\d+,\d+\]Reply\[\/pid\] Post by .+?\[\/b\]\x07*/gi,'')
	this.contentQuoteCache[arg.pId] = x.substr(0,250).replace(/<[^>]+$/,'')
	}


if(this.bbscode_pre && typeof(this.bbscode_pre)=='object'){
	for(var k in this.bbscode_pre)this.bbscode_pre[k](arg)
	arg.txt = arg.txt.replace(/<br\s*\/?>/g,'\x07')
	}
...
if(arg.removeQuote)
	arg.txt = arg.txt.replace(/\s*\[quote\]\[(?:tid|pid).+?\](?:Reply|Topic)\[\/(?:tid|pid)\] \[b\]Post by .+?\[\/quote\][\s\x07]*/,function($0){
		 return ''
		 })
...
if (arg.txt.match(/[\[=&\*`#]/)==null){
	...
	arg.txt = arg.txt.replace(/\x07/g,'<br/>')
	return arg.txt
	}
...
if(this.bbscode_aft && typeof(this.bbscode_aft) =='object'){
	for(var k in this.bbscode_aft)this.bbscode_aft[k](arg)
	}
...
var wo = window, commonui=wo.commonui, self = this, argsId = arg.argsId, noimg = arg.noImg|0
...
arg.txt = arg.txt.replace(/^(?:\[b\])?Reply to \[(pid|tid)=([,\d]+)\](?:Reply|Topic)\[\/(?:pid|tid)\] Post by (.+?\[\/uid\] \(\d+-\d+-\d+ \d+:\d+\):?)(?:\[\/b\])?\s*(\x07)*/gi,function($0,$1,$2,$3,$4){
if(arg.isComment)
	return "<span class='x'>"+$0+'</span>'
var i = $2.split(',')[0],z=self.contentQuoteCache
if(!z[i])
	return $0+($4?'':'\x07')
return '[quote]['+$1+'='+$2+']Reply[/pid] Post by '+$3+':\x07\x07\n'+z[i]+'\n[/quote]\x07'//加换行防止半个bbscode
})
...
//简化引用格式
arg.txt = arg.txt.replace(/(?:\[b\])?(Reply to )?(\[(?:t|p)id=[\d,\-]+\](?:Reply|Topic)\[\/(?:t|p)id\]) (?:\[b\])?Post by (\[uid=\d+\].+?\[\/uid\]) (\(\d+-\d+-\d+ \d+:\d+\)):?(?:\[\/b\])?/g,function($0,$1,$2,$3,$4){
	if(commonui.timeZoneFix)
		$4 = '('+commonui.timeZoneFix($4.substr(1,$4.length-2),'Y-m-d H:i')+')'
	return ($1?$1:'')+$2+" by "+$3+"<span class='xtxt silver' style='font-weight:normal'>"+$4+"</span>"
	})
...
if(arg.txt.indexOf('[album')!=-1){
	arg.txt = arg.txt.replace(/\[album=?([^\]]{0,50})\](.+?)\[\/album\]/gi,function ($0,$1,$2){
		...
		if (img[0]){
			...
			return "<table class='quote album'><tr><td><a href='./nuke/album.html?uid="+__CURRENT_UID+"&tid="+tid+"&pid="+pid+"&id="+id+"&a' target='_blank' onclick='ubbcode.album.open(event,\""+id+
				"\")'>"+$1+"\x07<img src='about:blank' onerror='this.parentNode._useloadread=48;"+(noimg?'':"ubbcode.album.imgLoad(this,\""+a+"\")")+"' style='display:none'/>\x07共"+img.length+"张图片 点击查看全部</a></td></tr></table>"
			}
		return "[album="+$1+"]"+$2+"[/album]"
		});//[album\]
	}//if

if(arg.txt.indexOf('[webgal')!=-1)
	ubbcode.webgal(arg)
...
if(arg.txt.indexOf('[flash')!=-1 || arg.txt.indexOf('[media')!=-1)
	this.parseFlash(arg)
...
arg.txt = arg.txt.replace(/\[s:(.{1,10}?)\]/gi,function($0,$1){
	...
	if(noimg&2097152)//小图片只有在调用指定时不显示
		return "<button alt='"+x+"' onclick='var tmp = _$(\"/img\",\"className\",\"smile"+z+"\",\"src\",\""+x+"\");this.parentNode.replaceChild(tmp,this)'/>"+y+"</button>"
	else
		return "<img class='smile"+z+"' src='"+x+"' alt='"+y+"' "+(ubbcode.smileLoadEx[arg.fId]?"onload='ubbcode.smileLoadEx[\""+arg.fId+"\"](this)'":'')+"/>"
	} );//[smile]
...
arg.txt = arg.txt.replace(/\[(tid|pid|stid|fid)=?([\d,]{0,50})\](.+?)\[\/\1\]/gi,function($0,$1,$2,$3){
...
arg.txt = arg.txt.replace(/\[uid=?(\d{0,50})\](.+?)\[\/uid\]/gi,function($0,$1,$2){
	if(parseInt($1,10)){
		if($2.length)
			return " <a href='/nuke.php?func=ucp&uid="+parseInt($1,10)+"' class='userlink b'>["+commonui.htmlName($2)+"]</a> "
		}
	else{
		if(parseInt($2,10))
			return " <a href='/nuke.php?func=ucp&uid="+parseInt($2,10)+"' class='userlink b'>[UID:"+parseInt($2,10)+"]</a> "
		else if($2.match(/anony_[a-f0-9]{32}/))
			return " <span class='userlink b'>["+commonui.htmlName($2)+"]</span>";
		}
	return $0
	})
...
arg.txt = arg.txt.replace(/\[@(.{2,20}?)\]/gi,function($0,$1){
	var i
	return" <a href='/nuke.php?func=ucp&__inchst=UTF-8&"+((i=$1.match(/^(?:UID)?(\d+)$/i)) ? "uid="+i[1] : "username="+encodeURIComponent($1))+"' class='userlink b'>"+$0+"</a> " 
	} );//[@]


arg.txt = this.parseTitle(arg)


if(arg.txt.indexOf('[chartradar')>-1)
...
return "<span style='display:inline-block;width:0.5em;height:0.5em;border-radius:0.25em;position:absolute;margin:"+(Math.random()*0.3)+"em 0 0 "+(Math.random()*0.3)+"em;color:white;background:red' title='bbscode "+t+" not match' onclick='alert(this.title)'> </span>"
}//



ubbcode.parseTitle = function(arg){

if(!arg.txt.match(/===|---|\[h\]|#/))
	return arg.txt

var tmp=0
, nts = " style='line-height:0;font-size:0;padding:0;margin:0 0 3px 0;height:0'"
arg.txt = arg.txt.replace(/\s*\[h\](.*?)\[\/h\]\s*\x07?/gi,function($0,$1){
	if(!$1)tmp++
	return "<h4 class='subtitle'"+($1?'':nts)+">"+$1+"</h4>"
	});//[h]

if(arg.opt & 65536)
	arg.txt = arg.txt.replace(/(^|>|\]|\x07)(#{1,10})(.{1,50}?)(?:$|\x07)/gi,function(no,pc,sp,tl){
		var tg = sp.length > 3 ? 'h5' : 'h4'
		return pc+"<"+tg+" class='subtitle'>"+tl+"</"+tg+">"
		});//[h]

arg.txt = arg.txt.replace(/(>|\]|\x07|^)\s*(?:(=|-){3})(.{0,100}?)(?:\2{3,})\s*(<|\[|\x07|$)/gi, function(no,pc,sy,tl,nc){
	if(tl.indexOf('\x07')!=-1)
		return no
	tl = tl.replace(new RegExp('^'+sy+'+|'+sy+'+$','g'),'')
	var tg = sy=='=' ? 'h4' : 'h5'
	if(tl=='')tmp++
	if(nc=='\x07')nc=''
	return pc+"<"+tg+" class='subtitle'"+(tl==''?nts:'')+">"+tl+"</"+tg+">"+nc
	})
...
if(arg.txt.length>24 
	&& arg.txt.charAt(arg.txt.length-1) == arg.txt.charAt(arg.txt.length-2)
	&& arg.txt.substr(arg.txt.length-50).match(this.continueCharProc.reg)
	)
	arg.txt = arg.txt.replace(this.continueCharProc.reg,function(a){
		var b = ''
...
;ubbcode.continueCharProc.reg = /[\xb7\x7e\x40\x23\x25\x26\x2a\x2b\x7c\x2d\x3d\x60\x7e\x21\x40\x23\x24\x25\x5e\x26\x2a\x28\x29\x5f\x2b\x7b\x7d\x7c\x3a\x22\x3c\x3e\x3f\x2d\x3d\x5b\x5d\x5c\x3b\x27\x2c\x2e\x2f\uff01\uffe5\u2026\u2026\uff08\uff09\u2014\u2014\uff5b\uff5d\uff1a\u201c\uff1f\u300b\u300a\u3010\u3011\u3001\uff1b\u2018\uff0c\u3002\u3001]{24,}/g
;
...
;(function(){
var table,tr,td,noboder,align,width,colspan,rowspan,space,bdrCo,bdrWd,ST,STACK,SPIT,cellpadding,currentArg,autoFit
init=function(){
table=/(?:\s*\x07\s*)?\[(\/?)(table|tr|td)(\s*[^\]\[]*)?\](?:\s*\x07\s*)?/gi
noboder=/noborder/
align=/top/g
...
bdrWd = 1
autoFit = __SETTING.bit&(4|8|16)
__NUKE.addCss(".ubbcode .tablecontainer {overflow:hidden} "
+" .ubbcode .tablecontainerfade::after { background-image: linear-gradient(90deg,"+__COLOR.bg0+"00 0%,"
		+__COLOR.bg0+"ff 100%);content:'\u00a0';height:100%;width:10%;position: absolute;} "
+" .ubbcode .tablecontainer .tablespacer {"+(autoFit?'width:60em;margin-right:-45em':'')+"} "
+" .ubbcode .tablespacer .tablespacer,.ubbcode .tablespacer .tablecontainer  {overflow:visible;width:auto;margin-right:auto}"
+".ubbcode .tblclickadj::after {width:10em;height:1em;font-size:1em;line-height:1em;display:inline-block;margin-top:-1em;background:"+__COLOR.postUiElm+";color:"+__COLOR.bg0+";content:'点击拖动查看全部'} "
+" .ubbcode .tablecontainer table {border:1px solid "+__COLOR.border3+";border-left:none;border-bottom:none;} "
+" .ubbcode .tablecontainer table td {border:1px solid "+__COLOR.border3+";border-top:none;border-right:none;} "
+" .ubbcode .tablecontainer table tr.th td {border-bottom-width:2px} "
)
},//
...
return "<div class='tablecontainer'><div class='tablespacer'><table id='"+ST.tableid+"' cellspacing='0px' style='"+z+";-add-style:here' lastId='"+ST.tableid+"'>"+(er?err('table'):'')
},//

tagTableEnd = function(ops,er){
ST.tablec--
var r = (er?err('table'):'')+"</table lastId='"+ST.tableid+"'></div></div><img src='about:blank' class='x' onerror='ubbcode.tableAdjSize(this.previousSibling)'/>"
STACK.pop()
...
var c=''
if(ops && ops.match(/(?:^|\s)th(?:$|\s)/))
	c += 'th'
return "<tr id='"+ST.trid+"'"+(c ? (" class='"+c+"'"): '')+">"+(er?err('tr'):'')
},//
...
return "<td id='"+ST.tdid+"' style='"+w+";-add-style:here' "+u+" lastId='"+ST.tdid+"'>"+(er?err('td'):'')
},//
...
err =function(t){
if(currentArg && currentArg.authorId && currentArg.authorId==__CURRENT_UID)
...
,tableMd2bb = function(txt){

var rx = /(?:^|\x07)\s*(?:\x07)\s*\|/g
,ry = /(?:\x07|$)\s*(\|)?/g
,start=false , end, x ,y, part=[],i=0
while((x = rx.exec(txt))!==null){
	start = x.index
	ry.lastIndex = rx.lastIndex
	while((y = ry.exec(txt))!==null){
		if(i++>1000){
...
if(part.length){
	...
	part.forEach(function(v){
		var tt = txt.substr(v[0]+of,v[1]-v[0]), fo=tt.length,off = tt.indexOf('|')
		,tb=[]
		tt.substr(off).split(/\x07/).forEach(function(v){
			v = v.trim().replace(/&#92;\|/g,'&#92;&#124;').replace(/&lt;\s*br\s*\/\s*&gt;/g,'\x07')
			if(v.charAt(0)=='|')
				v = v.substr(1)
			if(v.charAt(v.length-1)=='|')
				v = v.substr(0,v.length-1)
			if(v.match(/^[\s\-:\|]+$/))
				tb[tb.length-1] = tb[tb.length-1].replace(/^\[tr\]/, '[tr th]')
			else
				tb.push( '[tr][td]'+v.replace(/\|/g,'[/td][td]')+'[/td][/tr]')
			})
		tb=tt.substr(0,off)+'[table]'+tb.join('')+'[/table]'
		txt = txt.substr(0,v[0]+of)+tb+txt.substr(v[1]+of)
		of+=tb.length-fo
		})
	}
	console.log(txt)
...
ubbcode.tableAdjSize = function(o){
var sp = o.firstChild ,tb = sp.firstChild
if(autoFit){
	tb.style.width='auto'
	tb.style.tableLayout = 'auto'
	var ot = tb.offsetWidth
	,oa = ubbcode.getAvilWidth(sp)
	if(ot<oa){
		var to = ot + tb.firstChild.childNodes.length * __SETTING.fontSize
		tb.style.width = (to<oa ? to : oa)+'px'
		}
	if(tb.offsetWidth<sp.offsetWidth)
		sp.style.width=tb.offsetWidth+'px'
	}
if(o.offsetWidth<sp.offsetWidth){
	_$(o).$0('style','outline:'+__COLOR.docb+'px solid '+__COLOR.postUiElm+';outlineOffset:-'+__COLOR.docb+'px;'
		,'onclick',function(){
			if(this._scrol)return
			this._scrol=1
			this.style.outline='none'
			this.style.overflow='auto'
			this.className = this.className.replace('tblclickadj','')
			}
		)
	o.className+='  tblclickadj'
	}
}//

ubbcode.parseTable=function(arg){//#表格

if((arg.opt & 65536) && arg.txt.indexOf('|')>-1)
	arg.txt = tableMd2bb(arg.txt)

if(arg.txt.indexOf('[table')<0)
	return
...
SPIT = null
if(arg.opt&16){
	if((arg.opt&(8192|16384)) && !arg.txt.match(/\[span[\s=]aligntable\]/)){
		FLT = arg.opt&8192 ? 0 : 1//一列|两列
		SPIT={}
		}
	}
STACK = []
...
//del br between td/tr
arg.txt = arg.txt.replace(/\x07+<(td|tr|\/tr|\/table)/g,'<$1')
...
if(SPIT){
	for(var tableid in SPIT){
		arg.txt = arg.txt.replace(new RegExp("<table id='"+tableid+"'.+?<\\/table lastId='"+tableid+"'>"),function($0){//把表格切出来
					...
					for(var j=1;j<=tb.maxr;j++){
						...
						if(x[id])
							...
							$0 = $0.replace(new RegExp("<td id='"+id+"'.+?<\\/td lastId='"+id+"'>"),function($0){
								r.push($0.replace(/-add-style:here/,'display:block;width:auto').replace(/border-right:none;/,''))
								return ''
								})
							}
						}
				...
				$0 = m[0].replace(/-add-style:here/,'display:block').replace(/border-left:none;/,'border-left:none;border-right:none;')
...
//=============================
ubbcode.drawChartRadar=function(x){//#雷达图
...
if(!x || x==1){
	x=''
	h = "<span class='crimson'>用户因此帖中的发言被处罚"
	}
else if(x==3)
	h = "<span class='crimson'>被锁定账号发布的内容无法查看"
else if(x==2)
	h = "<span class='sienna'>用户在主题中被处罚"

return "<div class='lessernuke lessernuke"+x+"'>"+h+"</span> <a href='javascript:void(0)' onclick='var x = document.getElementsByName(\"lessernukeblk\");for(var i=0;i<x.length;i++){x[i].style.display=\"\"}'>点击查看</a><div style='display:none' name='lessernukeblk'>";}
...
var $ = _$
,x = $('custombg').firstChild
,z = x.firstChild
,c = z.getBoundingClientRect(), b= o.getBoundingClientRect()
,y = $('/div','id','extendtop','style','height:'+z.style.height+';margin-bottom:'+z.style.marginBottom+';overflow:hidden;opacity:0;transition:opacity 0.5s linear', $('/img','src',o.src,'style','margin-left:'+b.left+'px;margin-top:'+(b.top-c.top)+'px;width:'+b.width+'px;box-shadow: 0 0 1em black;'))

this.current = y

x.insertBefore(y,z.nextSibling)

setTimeout(function(){y.style.opacity=1},50)

if(x.lastChild.className.indexOf('overlay')!=-1)
	setTimeout(function(){commonui.triggerEvent(x.lastChild,'click')},500)
}//
,remove:function(o){
if(this.current){
	var s = this
	...
	return setTimeout(function(){
		if(s.current)
			s.current.parentNode.removeChild(s.current)
		s.current = null
		if(o)
			setTimeout(function(){s.load(o)},50)
		},550)
	}
}//
}///

ubbcode.parseFix = function(arg){//#fixsize标签
			...
			switch(y){
				...
				case 'background':
					if(z.match(/^(#[0-9a-fA-F]{3,6}|transparent)$/))
						b = z
					if(x[i+2] && x[i+2].match(/^(#[0-9a-fA-F]{3,6}|transparent)$/)){
						bb = x[i+2]
						i++
						}
				}
	...
	if(!fc)
		return
	arg.txt = arg.txt.replace(/\s*\x07/g,'\x00')
	...
	var extendtop = ((arg.opt&16)&&(arg.opt&32768)) ? 2 : 1

	arg.txt = arg.txt.replace(/\x00?\[(\/?)style(?:\s([^\]]+))?\]\x00?/g,function($0,$1,$2){
		if($1)
			return tg.length ? tg.pop() : $0
		var x = $2.split(/[,\s\x00]+/),t = {},s={display:'inline-block'},u={tagName:'div',style:s},rc=0,rs=0
		for(var i=0;i<x.length;i+=2){
			...
			switch(y){
				...
				case 'innerHTML':
					...
					break;
				case 'src':
					if(z.match(/^\.\/[a-zA-Z0-9_\/\.\-]+\.(?:jpg|jpeg|png|svg|webp)$/))
				...
				case 'extendtop':
					...
					u.onload+='ubbcode.extendtop.load(this,'+(z|1)+');'
					extendtop|=4
					break;
				case 'background':
				case 'color':
				...
				case 'background-repeat':
					s[y] = (z|0) ? 'repeat' : 'no-repeat'
					break
				case 'filter-drop-shadow':

					var pm,co=''
					z = z.replace(/#[0-9a-fA-F]{3,8}/,function(a){co = a;return ''})
					pm = z.match(/-?[\d\.]+%?/g)
					...
					if(pm && co){
						...
						if(co.length>6)
							co = 'rgba('+commonui.hexToRgba(co).join(',')+')'

						s['filter']='drop-shadow('
								+pm.join(' ')
								+' '+co
								+')'
						}
				...
				case 'dybg'://背景图缩放(相对于元素)x%,背景位置x,背景位置y,(活动量水平,活动量垂直 或 活动量水平)
					if(s.width && s.height){
						...
						if(pm && pb){
							...
							if(ubbcode.topTopicImgCheck(arg,pb))
								pb=''
		...
		if(u.src){
			tg.push('')
			u.tagName= "img"
			if(ubbcode.topTopicImgCheck(arg,u.src))
				u.src=''
			}
		else
			tg.push('</div>')
...
/*
	...
	if(arg.authorId && arg.authorId==__CURRENT_UID){
		while(tg.length)
			arg.txt +=tg.pop()+ubbcode.errorHint('fixsize,style')
		}
	if(arg.opt&(16|4096)){
		arg.txt = arg.txt.replace(/\[spine\](.+?)\[\/spine\]/g,function($0,$1,$2){
			var e, x = ubbcode.unSecureText($1)
			try{
			x = JSON.parse(x.replace(/[\x00\u00A0\u2002\u2003]/g,''))
			}catch(e){console.log('JSON ERR',e)}
			if(x){
				x.atlasPath='//'+__RES_BASE+x.atlasPath.substr(1,255)
				x.skelPath='//'+__RES_BASE+x.skelPath.substr(1,255)
				if(!arg.spinearg)arg.spinearg={}
				var i = 'spine'+Math.random()
				arg.spinearg[i]=x
				return "<img src='about:blank' onerror='ubbcode.loadSpine(this)' data-argsid='"+arg.argsId+"' data-spineid='"+i+"'/>"
				}
			return '';
			})
		}
	...
	arg.txt = arg.txt.replace(/\x00/g,'\x07')
	...
	if((extendtop&4)==0 && ubbcode.extendtop.current)
		arg.txt +="<img src='about:blank' style='display:none' onerror='ubbcode.extendtop.remove()'/>"
}//fe


ubbcode.loadSpine=function(o){
var x = ubbcode.bbscodeConvArgsSave[o.dataset.argsid].spinearg[o.dataset.spineid]
__SCRIPTS.asyncLoad(__IMG_BASE+'/proxy/cache_attach/cross_domain_tmp/spine.js',function(){
	var p = o.parentNode
	x.x *= p.offsetWidth
	x.y *= p.offsetHeight
	var container = _$('/div','style','width:'+p.offsetWidth+'px;height:'+p.offsetHeight+'px;transform:scale(1, 1);transform-origin:left top;',_$('/canvas','style','width:100%;height:100%'))
	o.parentNode.replaceChild(container,o)
	o=null

	x.onSkelLoaded= function(skel){
		console.log(skel);
		}
	x.onSuccess= function(widget) {

		widget.config._playSpecial=x._playSpecial
		widget.config._clickInteract=x._clickInteract
		widget.config._PrevSpecialStartTime=0
		widget.config._isPlayingInteract=false

		console.log(widget);

		widget.canvas.oncontextmenu = function(){return false;};
		widget.state.addListener({
			end: function(e) {
				if (e.animation.name == widget.config._clickInteract)
					widget.config._isPlayingInteract = false;
				},
			complete: function(e) {
				if (widget.config._playSpecial && 
					Date.now() - widget.config._PrevSpecialStartTime >= 8 * 1000 &&
					Math.random() < 0.3
					) {
						widget.config._PrevSpecialStartTime = Date.now();
						console.log("play special");
						var entry = widget.state.setAnimation(0, widget.config._playSpecial, false, 0);
						entry.mixDuration = 0.5;
						widget.state.addAnimation(0, widget.config.animation, true, 0);
						}
				}
			});
			container.addEventListener("click", function(){
				if (widget.config._isPlayingInteract || !widget.config._clickInteract)
					return;
				widget.config._isPlayingInteract = true;
				var entry = widget.state.setAnimation(0, widget.config._clickInteract, false, 0);
				entry.mixDuration = 0.5;
				widget.state.addAnimation(0, widget.config.animation, true, 0);
				});
		},
	SpineRender.render(container, x);

	})
}//

//置顶图片检查
ubbcode.topTopicImgCheck = function(arg,src,srco){
if((arg.opt&16) && arg.inTopImg){//在置顶中
	var x = 0
	if(!arg.inTopImg[src]){
		arg.inTopImg[src] = 1
		if(!srco)
			srco = ubbcode.attach.check(arg.i,src)
		arg.inTopImg[0]+=srco.size
		if(arg.inTopImg[0]>1200){
			arg.inTopImg.log0 = '置顶贴图片合计不超过1200k (已用'+arg.inTopImg[0]
			x=  1
			}
		}
	if(arg.opt&(32|4096) && (arg.inTopImg[1]+=srco.w*srco.h) && arg.inTopImg[1]>(arg.opt&4096 ? 400000 : 200000)){//长置顶
		arg.inTopImg.log1 = '置顶贴图片合计不超过'+(arg.opt&4096 ? 400000 : 200000)+'像素 (已用'+arg.inTopImg[1]
		x= 1
		}
	if(x){
		if(!arg.inTopImg.to)
			arg.inTopImg.to = setTimeout(function(){console.log(arg.inTopImg)},3000)
		return 1
		}
	}
}//
...
/**
 * 
 * @param {*} opt -1生成QR码 0指定使用按钮 1~100设置百分比宽度
 * @param {*} src 
 * @param {*} noimg &1点击按钮显示图片 &2同上 &4返回obj
 * @param {*} arg 
 */
ubbcode.imgGen = function(opt,src,noimg,arg){//#图片标签
//处理选项
if(opt=='-1'){//-1 生成QR码
...
var arr = {
	...
	'data-argi':arg.i,
	'data-srcorg':'',//大图
	'data-usethumb':'',//选择的thumb 有此项时src和宽高使用此项
	'data-srclazy':'',//慢加载
	'data-nw':0,//宽
	'data-nh':0,//高
	'data-apporg':'',
	'data-appthumb':''
	},tmp,w='',n='',wl=(arg.maxWidth|0) ? (arg.maxWidth) : 688

if(!arg.fontsize){
	tmp = arg.maxWidthO?arg.maxWidthO:arg.c
	arg.fontsize = tmp ? parseInt(commonui.getStyle(tmp,'font-size')) : __SETTING.fontSize
	}
wl-=arg.fontsize*3.2//保险宽度减去一个quote的留白

opt = opt|0
if(opt>0 && opt<=100)//1~100设置百分比宽度
	arr.style+="width:"+(opt-0.1)+"%;";

if(arg.opt&2){}//in [fixsize]
else{
	arr.style+="max-height:1em;" 
	//w+="max-width:90%;" 
	arr.onload+='ubbcode.adjImgSize(this,'+wl+');'
	}
if(arg.opt&256)
	arr.onload+='ubbcode.bbscodeConvArgsSave["'+arg.argsId+'"].onResize();'
...
if(src && src.charAt(0)=='.'){
	src = src.replace(/^\.(\d+)?\//,function(a,b){
		if(b && arg.urlReplace && arg.urlReplace[b-1])
			return arg.urlReplace[b-1]+'/'
		return commonui.getAttachBase(src)+'/'
		})
	if((arg.opt&128) && src.charAt(0)=='.' && commonui.userfile2Long){
		src = commonui.userfile2Long(src)
		if(src.charAt(0)!='.')
			arr._us=1
		}
	}
...
//确定显示状态
var alt='',a, srco,dis,dw,dh,orgApp='',thApp='',dbg=''; // true显示按钮 null显示原图

if (
	((arg.opt & 128) ? this.checkSigImg(src) : (commonui.ifUrlAttach(src)==2))
	&& (a = this.attach.check(arg.i,src))
	){//附件or签名upload

	if(a.type!='img'){
		if(a.type=='video' && (a.ext=='mp.mp4'||a.ext=='gif.mp4'))
			return '[flash=video]'+a.autoUrl.replace(/\.(org|medium|thumb)\.jpg$/,'')+'[/flash]'
	...
	if(tmp){//用户指定使用缩略图
		arr['data-srcorg']  = src.replace(/\.(?:thumb\.|thumb_s\.|thumb_ss\.|medium\.)[a-zA-Z0-9]+$/,"")
		if(a.type=='img' && a.w && a.w<=dw && a.size && a.size<200 && (noimg&1)==0){//原图和缩略图一样大 并且小于300k 使用原图
			src = arr['data-srcorg']
			dw = a.w
			dh = a.h
			}
		...
		if(a.size>100)
			this.imgGen.c++
		var dis = this.attach.ifUseThumb(noimg, arg.opt, a, this.imgGen.c)
		dw = a.w
		dh = a.h
		arr['data-srcorg'] = src
		if(dis && dis!==true){
				arr['data-usethumb'] = src+dis
				if(dis.indexOf('medium')!=-1){
					dw = 640
					dh = dw * dr
					}
				else{
	...
	if((arg.opt&16) && ubbcode.topTopicImgCheck(arg,src,a)){//置顶超限
		src='about:blank'
		arr.style=this.imgGen.s
		}

	//if(thumb&64)
	//	arr['data-appthumb'] = src+'.medium.jpg'

	}
else{//不是附件
	if(!commonui.checkOtherImg(src))
		return src
	...
	if(arg.opt&16){//在置顶中
		if(arg.fId==706 && src.match(/^https?:\/\/image\.sinajs\.cn\/newchart\/min\//)){}//股票图
		else{
			src='about:blank'
			alt='置顶必须\n使用附件'
			arr.style=this.imgGen.s
...
if(noimg&4)
	return {src:src, srcoo:srcoo, srct:'', w:dw, h:dh }
//alt+=dbg
if(window.__APPEMBED){
	arr['data-apporg']=arr['data-srcorg']
	arr['data-appthumb']=arr['data-usethumb']
	this.insertAllImg(arg.i,arr['data-apporg'],arr['data-appthumb'])
	}

arr.alt = alt

if(arr['data-usethumb'] )
	src = arr['data-usethumb']

if(dis!==true){
	if(window.__INSECTOB){
		arr.onerror+='ubbcode.imgError(this)'
		arr['data-srclazy'] = src
		arr.src='about:blank'
		}
	else
		arr.src = src
	
	return  this.imgGenJoin(arr)
	}
else{
	arr.src = arr['data-srcorg']
	var img = this.imgGenJoin(arr), ck = ++this.imgGenManualC
	this.manualLoadCache[ck]=img//.replace(/max-width:\d+px/,'max-width:90%')
	//if(dis === true)
	return "<button type='button' onclick='this.nextSibling.innerHTML=ubbcode.manualLoadCache[\""+ck+"\"];this.nextSibling.style.display=\"\";this.style.display=\"none\"'>显示图片"
	+(a.size>1024 ? '('+Math.round(a.size/1024*10)/10+'M)' : '('+a.size+'K)')
	+"</button><span style='display:none'></span>"
	//else if(dis)
	//	return "<a href='javascript:void(0)' class='thumblink' onclick='this.nextSibling.innerHTML=ubbcode.manualLoadCache[\""+ck+"\"];this.nextSibling.style.display=\"\";this.style.display=\"none\"'><img "+(w?" style='"+w+"'":'')+(n?" onload='"+n+"'":'')+" src='"+src+dis+"' alt='"+alt+"' onerror=''/></a><span style='display:none'></span>"
	}
}//fe

ubbcode.imgGenManualC = 0;

ubbcode.imgGenJoin = function(arr){
var img = "<img "
for(var k in arr)
	img+= " "+k+"='"+(arr[k]+'').replace(/'/g,'%27')+"' "

img+='/>'
return img
}//

ubbcode.imgGenSwap=function(o){
var s = o.getAttribute('data-clickorg')
if(s) o.src=s
}//
...
if(o.getAttribute('data-srclazy'))
	var h 
	if(h=(o.getAttribute('data-nh')|0)){
		var w = o.getAttribute('data-nw')|0, aw = this.getAvilWidth(o)
		if(w>aw)
			h = aw/w*h, w=aw
		o.style.minHeight = h+'px'
		o.style.minWidth = w+'px'
		console.log('set minH '+h)
		}
...
//图片尺寸调整
//=============================
ubbcode.adjImgSize = function(o, wl){
if(o.src=='about:blank')return
var aw = this.getAvilWidth(o)
commonui._debug.push(o.src+' '+aw+' '+wl)
if(!aw)
	aw = wl
o.style.maxWidth = aw+'px'
o.style.maxHeight = 'none'
if(o._nw = (o.getAttribute('data-nw')|0)){}
else
	o._nw = o.naturalWidth ? o.naturalWidth : this.calcNatureWidth(o)

o._zopt = 0
if(aw<o._nw)
	o._zopt |= 4
var og = o.getAttribute('data-srcorg')
if(og && og!=o.src)
	o._zopt |= 1
...
if(o._zopt){
	o.style.outline=__COLOR.docb+'px solid '+__COLOR.postUiElm
	o.style.outlineOffset='-'+__COLOR.docb+'px'
	o.onclick = function(e){
		var s
		if(window.__APPEMBED){
			if(s = this.getAttribute('data-apporg')){
				ubbcode.embedCallOpenPic(
					this.getAttribute('data-argi')
					,s
					,this.getAttribute('data-appthumb'))
				commonui.cancelBubble(e)
				return commonui.cancelEvent(e)
				}
			}
		if(this._zopt & 1){//srcorg是原图 点击时替换src onload后再click一次
			this.src = this.getAttribute('data-srcorg')
			this._dozoom = 1
			this.setAttribute('data-nw','')
			this.setAttribute('data-nh','')
	...
	o._zoom = function(){
		...
		if(s.maxWidth=='none'){
			...
			s.outlineWidth=__COLOR.docb+'px'
			s.outlineOffset = '-'+__COLOR.docb+'px'
...
ubbcode.calcNatureWidth = function(img){
	x = new Image()
	x.src = img.src
	return x.width
...
ubbcode.embedCallOpenPic = function(argi,ourl,turl){
var parg = commonui.postArg ? commonui.postArg.data[argi] : {}
if(parg.allImg){
	var qq =parg.allImg, pq=[]
	for(var i=0;i<qq.length;i++){
		pq.push({
			'init': qq[i].thumb ? qq[i].thumb : qq[i].org
			,'org': qq[i].thumb ? qq[i].org : null
			,'orgs': qq[i].orgs ? qq[i].orgs : null
			,'show': qq[i].org == ourl ? 1 : 0
			,'type': qq[i].type ? qq[i].type : 'img'
			})
		}
	}
else{
...
__doAction.appDoSync('openPic', {'urls':pq, 'opt':0})
}//
...
ubbcode.insertAllImg = function(i,urlorg,urltmb,type,urlstatic){//type: video / img
var tmp
if(commonui.postArg && ( tmp = commonui.postArg.data[i])){
	if(!tmp.allImg)
		tmp.allImg=[]
	tmp.allImg.push({
		'thumb':urltmb
		,'org':urlorg
		,'orgs':urlstatic?urlstatic:''
		,'type':type ? type : 'img'
		})
	}
}//
...
_$(o)._.on('copy',function(){
	...
	if(a.pId)
		t+="&pid="+a.pId+"&to=1"
	if(t && commonui.range){
		var y=commonui.range.get(),x = y.getHtml(),z = x.match(/\x07/g)
		if(!z || z.length<4)
			return;
		x = _$('/span').$0('style','position:fixed;right:-9999px','innerHTML',y.getHtml()+'\x07\x07'+location.protocol+'//'+location.host+'/read.php?'+t)
...
if(!this.upup){
	if(!this.upupProc.loading){
		...
		loader.script(__COMMONRES_PATH+'/js_upup.js?'+__SCRIPTS.rand, function(){ubbcode.upupProc()}, 0, 0)
		}
	return
	}
...
//=============================
//嵌套引用
ubbcode.parseQuote = {//#引用标签
p:false,
a:null,
md2bb:function(n){
var lv = 0,add=function(l){
var r = '',j
if(l>lv){
	j=l-lv
	for(i=0;i<j;i++)
...
n=n.replace(/(\x07)(\s*)((?:&gt;){0,3})/g,function(a,x,y,z){
	if(z.length>lv || z.length<lv)
		return x+y+add(z.length>>2)
	return a
	})
return n
},//
parse:function(arg){
	...
	if((arg.opt & 65536) && arg.txt.indexOf('&gt;')>-1)
		arg.txt = this.md2bb(arg.txt)
	
	this.p = arg.txt.indexOf('[quote')+1
	if(this.p)
		arg.txt = arg.txt.replace(/\x07?\s*(\[\/?quote\])\s*\x07?/gi,"$1")
	var limit=0
	this.a = arg
	while (this.p){
		if(limit++>5)break;
	...
	return txt.replace(/\[quote\]([^\x00]+?)\[\/quote\]/gi,function($0,$1){//  多行匹配
			...
			if($1.match(/^\s*\[(?:tid|pid)=[\d,]+\](?:Reply|Topic)\[\/(?:pid|tid)\] (?:\[b\])?Post by/)){
				return "<div class='quote' "+s+">\n"+
						$1.replace(/^\s*\[(?:tid|pid)=[\d,]+\](?:Reply|Topic)\[\/(?:pid|tid)\] \[b\]Post by.+?\x07\x07(\[b\]Reply to .+?\[\/uid\] \(\d+-\d+-\d+ \d+:\d+\):?(\[\/b\])?)/,function($0,$1){
				...
				if(self.a.c){
					for(var i=0,y,z,x=self.a.c;i<3;i++){
						if(x.parentNode){
							if(x.parentNode.className.indexOf('postrow')!=-1 
								&& (y=commonui.getRGBFromStyle(commonui.getStyle(x,'background-color')))
								&& y[0]!==0
								){
...
exe:function(txt,count){
	this.count = count
	var self = this
	return txt.replace(/\x07?\[list(=.)?\](.*?)\[\/list\]\s*\x07?/gi,function($0,$1,$2){//  多行匹配
			if($2.match(/\[list(?:=.)?\]/i))
				return '[list'+($1?$1:'')+']'+self.exe($2+'[/list]',true)
			$2 = $2.split('[*]')
			var l = ''
			for(var i=0;i<$2.length;i++){
				$2[i] = ubbcode.trimbr($2[i])
				if($2[i])
					l += '<li>' + $2[i] + '</li>'
				}
			if($1)
...
ubbcode.trimbr = function(txt,opt){//&1 rightonly  &2 leftonly
var j,k
if((opt & 1)==0){
	k=1
	j = txt.length
...
if((opt & 2)==0){
	...
	for(var i=txt.length-1;i>=j;i--){
		...
		if(c==7){
			...
			if(c==62 && k>0 && txt.substr(i-5,6)=='<br />'){
				i-=5
				k--
				}
			else{
...
if(arg.txt.indexOf('[randomblock')!=-1){
	arg.txt = arg.txt.replace(/\x07{0,2}\s*\[randomblock\](.+?)\[\/randomblock\]\s*\x07{0,2}/gi,function($0,$1){
		$1 = ubbcode.trimbr($1)
		if(!arg.randomBlock)arg.randomBlock=[]
		var hax = $1.indexOf('[fixsize')!=-1 ? 1 :0
		arg.randomBlock.push($1)
		arg.collapseAttach=1
		return arg.randomBlock.length>1 ? '' : "<div class='collapse_content' style='"+(hax ? 'clear:both;':'')+"height:0px;overflow:hidden'></div><img src='about:blank' class='x' onerror='ubbcode.collapse.loadr(this.previousSibling,\""+arg.argsId+"\")'/>";
		})
	}
},//fe
...
if(!o._selector){
	...
	for(var i=0;i<arg.randomBlock.length;i++)
		rs._.add($('/a').$0('href','javascript:void(0)','name',i,'className','block_txt','style','paddingTop:0;paddingBottom:0;lineHeight:1em;borderRadius:0.5em;margin:0 1em;background:'+(i==id ? __COLOR.postUiElm_l : __COLOR.postUiElm),'innerHTML','&emsp;','onclick',function(){
				if(o._selected == this.name)return
				for(var i=0,j=this.parentNode.childNodes;i<j.length;i++)
					j[i].style.background = i==this.name ? __COLOR.postUiElm_l : __COLOR.postUiElm
 ...
 * @returns [
 *				{start,end}, //txt node
 *				{tag,start,end}, //tag node
 *				{start,end}, 
 *				 ...
 *				 ]
 */
ubbcode.regGetBlock = function(re,txt){
var m , j =0,c = [],cc=[],i=0
while ((m = re.exec(txt)) != null) {
	if(m[0].charAt(1)=='/'){//end
		if(j){
			j--
			c.push(m.index,m[0].length)
			}
		}
	else{//start
		if(j==0){
			j++
			c.push(m[0],m.index,m[0].length)
			}
		}
	}
console.log(c)
...
for(var i=0;i<c.length;i+=5){
	if(c[i+3]){
		cc.push({start:j, end:c[i+1]})
		cc.push({tag:c[i+0], start:c[i+1]+c[i+2], end:c[i+3]})
		j=c[i+3]+c[i+4]
		}
	}
if(j)
	cc.push({start:j, end:txt.length})
return cc
}//
...
ubbcode.webgal = function(arg){

var cut = this.regGetBlock(/\[\/?(webgal|resgroup)\]/g , arg.txt)
if(!cut.length)
	return

var newtxt = '',imgset={},j=0
cut.forEach(function(c){
	if(c.tag=='[resgroup]'){
		var tx = arg.txt.substring(c.start,c.end)
		tx = ubbcode.unSecureText(tx,1)
		tx = tx.replace(/\[\/?img\]/g , '')
		tx = tx.replace(/([^\x07\s]{1,20})\s+(\.\/mon_\d+\/\d+\/[^\.]+\.webp)/g, function(a,b,c){
			imgset[b] = commonui.getAttachBase(c)+'/'+c.substr(2)
			return '[color=firebrick]'+b+'[/color] [color=seagreen]'+c+'[/color]'
			})
		c.txt = tx
		}
	})
console.log(cut,imgset)
cut.forEach(function(c){
	if(c.tag){
		if(c.tag=='[webgal]'){
			var tx = arg.txt.substring(c.start,c.end)
			tx = ubbcode.unSecureText(tx,1)
			tx = tx.replace(/([^\s:]{1,20})\.(jpg|png|gif|webp)/g, function(a,b){
				if(imgset[b])
					return imgset[b]
				return a
				})
			var bg = 'data:image/svg+xml;utf8,'+ encodeURIComponent('<svg width="4em" height="4em" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 100 100"><circle cx="50" cy="50" r="50"  style="fill:'+__COLOR.postUiElm+'"/><path d="M83.7 50 31.122 80.358V19.642Z" style="fill:'+__COLOR.bg0+'"/></svg>')
			, id = arg.tId+'_'+arg.pId+'_'+(j++)
			newtxt += "<div class='video' style='display:none;width:640px;height:480px'><div class='spacer' style='width:100%;height:100%;'><a href='javascript:void(0)' style='display:block;height:100%;background:url("+bg+") no-repeat 50% 50%' onclick='ubbcode.webgal.load(this.parentNode.parentNode,\""+id+"\")'></a></div></div><img src='about:blank' class='x' onerror='ubbcode.webgal.setSize(this.previousSibling)'/>"
			ubbcode.webgal.data[id] = JSON.stringify({
				event:'debug'
				,data:{
					command:6//TEMP_SCENE
					,message: tx
					}
				})
			}
		else if(c.tag=='[resgroup]'){
			newtxt += '\n[collapse=文件资源]\n'+c.txt+'\n[/collapse]\n'
			}
		}
	else
		newtxt += arg.txt.substring(c.start,c.end)
	})
...
ubbcode.webgal.setSize = function(o){
var x = ubbcode.getAvilWidth(o)*0.9
if(x>800)x=800
o.style.width = x+'px'
o.style.height = (x/4*3)+'px'
o.style.display = ''
}//

ubbcode.webgal.load = function(o,id){
o.innerHTML = ''
o.appendChild(_$('/iframe','style','border:none;width:100%;height:100%','src','https://img.nga.178.com/misc/webgal/index.html','onload',function(){
	this.contentWindow.postMessage(ubbcode.webgal.data[id], location.protocol+'//img.nga.178.com')
	}))
}//
...
ubbcode.codeTag={//#code标签

parent:ubbcode,
highlighter:{
	lua:"/js_highlighter.lua.js?"+__SCRIPTS.rand,
	php:"/js_highlighter.php.js?"+__SCRIPTS.rand,
	c:"/js_highlighter.c.js?"+__SCRIPTS.rand,
	js:"/js_highlighter.js.js?"+__SCRIPTS.rand,
	javascript:"/js_highlighter.js.js?"+__SCRIPTS.rand,
	xml:"/js_highlighter.xml.js?"+__SCRIPTS.rand,
	css:"/js_highlighter.css.js?"+__SCRIPTS.rand,
	py:"/js_highlighter.py.js?"+__SCRIPTS.rand
	},
...
md2bb : function(txt){
var ri={on:1,ix:0, rg : /(^|\x07)\s*```([a-zA-Z0-9]{0,20})?(?=[^a-zA-Z0-9])/g} ,i=0
while((x = ri.rg.exec(txt))!==null){
	if(ri.on==1){
		ri[i] = ri.on = new Number(2)
		ri[i].lang = (x[2] ? x[2] : 'c')
		}
	else
		ri[i] = ri.on = 1
...
if(ri[i-1]==2)
	ri[i-1]=0
i=-1
txt = txt.replace(ri.rg,function(x,a,b){
	++i
	if(ri[i]==2)
		return a+'[code='+ri[i].lang+']'
	else if(ri[i]==1)
		return a+'[/code]'
	return x
	})
...
parse:function(arg){
var self = this
if(arg.txt.indexOf('[code')!==-1){
	var od = 0 //1:run here 2:run in sandbox 4:run instant 
	, pt = commonui.postArg ? commonui.postArg.data : null
	if(pt && pt[arg.i] && pt[arg.i].atItem){
		var pat = pt[arg.i].atItem
		if(pat.i(28)!==false)
			od = 1
		else if((arg.opt&512) || pat.i(26)!==false)
			od = 2
		}

	var m , j =0,cut = []//type,startpos,startlen,endpos,endlen
	while ((m = this.re.exec(arg.txt)) != null) {
		if(m[1]){//end
			if(j){
				j--
				cut.push(m.index,m[0].length)
				}
			}
		else{//start
			if(j==0){
				j++
				cut.push(m[2],m.index,m[0].length)
				}
			}
		}
	...
	for(var i=0; i<cut.length; i+=5){
		var lng = cut[i], spos=cut[i+1], slen=cut[i+2], epos=cut[i+3], elen=cut[i+4]
		if(lng)lng = lng.substr(1)
		if(!this.highlighter[lng])lng='c'
		if(!arg.blockCode)arg.blockCode=[]
		var onecode = arg.txt.substring(spos+slen,epos)
		arg.blockCode.push([ lng, onecode ])
		var ck = arg.blockCode.length-1
		newtxt.push(arg.txt.substring(j,spos))
		if((lng=='javascript'||lng=='js') && od){
			newtxt.push(self.viewCodeJs(
				arg.argsId, ck, lng
				, onecode.substr(0,50).indexOf('//instantrun')!=-1 ? (od|4) : od 
				))
			}
		else
			newtxt.push( self.viewCode(null, arg.argsId, ck, lng, arg.i))
		
		j = epos+elen
		}

	newtxt.push(arg.txt.substring(j))
...
viewCodeJs:function(argid,ck,lng,od){
var id = argid+'_'+ck
var ret = '<div class="video" style="background:none;border-color:'+__COLOR.border2+';"><div style="background-color:'+__COLOR.border2+';">Javascript'+(od==1?'嵌入':'沙盒')+' 此处程序由发帖者提供 <button onclick="ubbcode.codeTag.viewCode(this.parentNode.parentNode,\''+argid+'\','+ck+',\''+lng+'\')">查看源码</button></div>'
if(od&1){
	ret += '<div class="runCodeHere" id="runcodehere'+id+'">'
	if(od&4){
		var tmi = _$('/img'), tmc=_$('/div')._.add(tmi)
		ubbcode.codeTag.runCodeHere(tmi,argid,ck)
		}
	else
		ret+='<img src="about:blank" style="display:none" onerror="ubbcode.codeTag.runCodeHere(this,\''+argid+'\','+ck+')"/></div>'
	
	}
else
	ret += '<iframe id="runcode'+id+'" style="border:none;height:'+this.resizeDef+'px" src="'+location.protocol+'//img.nga.178.com/misc/bbscodesandbox/sandbox.html" onload="ubbcode.codeTag.runCode(this,\''+argid+'\','+ck+')" onmouseover="ubbcode.codeTag.resizeSend(this,\''+argid+'\','+ck+')"></iframe>'

ret += '</div><img src="about:blank" style="display:none" onerror="this.previousSibling.style.width=(ubbcode.getAvilWidth(this.previousSibling)*0.9)+\'px\'"/>'
return ret
},//

viewCode : function(o,argsId,id,l,lo){
var arg = this.parent.bbscodeConvArgsSave[argsId]
, r = "<div><span class='orange'>Code <span class='gray'>"+l+((l=='javascript'||l=='js') ? ' (运行沙盒需版主'+(__GP.admincheck ? ' <a href="javascript:void(0)" class="b" onclick="commonui.atItemBuyUse(event,'+arg.tId+','+(arg.pId|0)+')">设置</a>)':'设置') : '')+"</span>:</span>\x07<div class='textfield'></div><img src='about:blank' onerror='ubbcode.codeTag.loadCode(this.previousSibling,\""+argsId+"\","+id+")' class='x'/></div>"
if(o)
...
runCodeHere:function(o,argsId,id){
var a = this.parent.bbscodeConvArgsSave[argsId],f = "runHereBlock_"+argsId+"_"+id, p=o.parentNode
,tmp = "window.ubbcode.codeTag."+f+"=function(o,argid,id){\n"+this.parent.unSecureText(a.blockCode[id][1], true)+"\n}"
try{
	console.log('ubbcode.codeTag.runCodeHere',tmp)
	eval(tmp)
	if(window.ubbcode.codeTag[f])
		setTimeout(function(){window.ubbcode.codeTag[f](p,argsId,id)})
	}
catch(e){
	console.log(e)
	}
...
var a = this.parent.bbscodeConvArgsSave[argsId], u = ubbcode.urlToAry(o.src)
...
var self = this , arg = this.parent.bbscodeConvArgsSave[argsId], l = arg.blockCode[id][0], parser = function(){
	...
	var txt = arg.blockCode[id][1].replace(/\u2003{2}/ig,"    ")
	r =Highlighter.Execute(self.parent.unSecureText(txt, true), l)
	r = r.replace(/\/\/add-bbscode-code-(start|end)-here\/\//g,function(x,y){return y=='start' ? '<b>[code]</b>'+x : '<b>[/code]</b>'+x})
	_$(o).$0('style','width:auto;height:auto;overflow:auto;max-width:'+(ubbcode.getAvilWidth(o)*0.9)+'px;max-height:'+((__SETTING.bit & __SETTING.bits.size7) ? '50em' : '500em'))
	o.innerHTML = r
	}

if (!window.Highlighter)
	loader.script(__COMMONRES_PATH+"/js_highlighter.js?"+__SCRIPTS.rand,
		function(){
...
//=============================
//tid pid uid
ubbcode.postLinkTag=function(tag,arg,txt,more){//#内部链接标签
		...
		for(var i = 0;i<j;i++){
			...
			pcc = $('/div',
					...
					$('/div','className','c2','style',{padding:'6px'}
						,'_avilWidth', (ao ? (ubbcode.getAvilWidth(ao)*0.9) : 0)
...
//ubi
;(function(){
var g = /(\s*\x07?\s*)(?:\[(\/?)(b|color|size|align|del|font|u|i|sub|sup|span|upup|l|r)(?:[=\s]+([^\]]+))?\])(\s*\x07\s*)?|\n+/gi,b={},off,aa,
...
if(e){
	if(b[t]>0)
		...
		if(aa.authorId && aa.authorId==__CURRENT_UID)
			return ubbcode.errorHint(t)
		else
			return ''
		}
...
if(t=='del')
	...
	return e? "</span>" : "<span style='font-style:italic;letter-spacing:0.1em'>"
...
ubbcode.ubi={
md2bb:function(n){
var i=0,j = 0,lo = [],remove=[]
n = n.replace(/\*{2,4}|\x07/g,function(x,li){
	if(x.charAt(0)=='<' || x.charAt(0)=='\x07'){
		if(j){
			j=0
			remove.push(lo)
			}
...
if(remove.length)
	remove.forEach(function(r){
		n = n.substr(0,r[0])+'***'+n.substr(r[1])
		})
return n
},//
parse:function(arg,opt){//#U B I 文字标签
var n = ''
off={}
b={}
if(arg.constructor==String)
	arg={txt:arg, authorId:0, opt:opt|0}
n = arg.txt
aa=arg
if((arg.opt & 65536) && n.indexOf('**')>-1)
	n = this.md2bb(n)
...
//=====================
ubbcode.autoplayVideo = function(e,o){
if(e.type=='play'){
	o._pl=(o._pl|0)+1
	if(o._pl>5 && window.__APPEMBED){
		ubbcode.embedCallOpenPic(
			o.getAttribute('data-argi')
			,o.src
			,null
			)
		o.pause()
	...
	if(window.performance)
		o._stopTime = performance.now()
	setTimeout(function(){o.controls=0},2000)
	}
else if(e.type=='click'){
	if(!o.controls)
		o.controls = 1
	setTimeout(function(){
		if(o._stopTime && (performance.now()-o._stopTime)>120)
			o.play()
		},10)//click->pause->timeout
	}
}//


ubbcode.parseFlash = function(arg){//#视频标签处理
var self = this
arg.txt = arg.txt.replace(/\[(flash|media)(?:=(video|audio))?\](.{1,512}?)\[\/\1\]/gi,function($0,$1,$typ,$2){
	var w=0, h=0,x,i,uu,p0=location.protocol
	,u= $2.replace(/\.bilibili\.(?:us|tv)/,'.bilibili.com')
	...
	if(commonui.ifUrlAttach(u)){
		var sr='',tvb =0,att
		u = u.replace(/\?filename=.+$/,function(a){sr = a;return ''})//remove org name
		att = self.attach.check(arg.i,u)
		if(att){
			if(att.type=='video'){
				if(att.ext=='gif.mp4')
					tvb=1
				else if(att.ext=='mp.mp4')
					tvb=16
				else{
					if(arg.fId==-7)//temp202004
						tvb=2
					else
						tvb=8
					}
		...
		if((tvb&17) || ((tvb&8)&&att.size<1024))
			tvb |=32
		else if(sr && sr.indexOf('autoplay')>-1){
			try{if(commonui.userInfo.users[arg.authorId].group_bit_buf & 2097152)
					tvb |=32
				}catch(sr){}
			}

		if(tvb&25){//1 8 16
			var tmb = self.attach.thumbPx(att.thumb)
			tmb = tmb ? (att.url+tmb) : ''
			var vo = "<video playsinline preload=none muted=true src='"+att.url+"' "+(tvb==1?'loop':'')+" poster='"+tmb+"' "+((tvb&32)?"data-playlazy='1'":'controls')+" style='display:none;background:"+__COLOR.gbg1+";width:"+(att.w?att.w:200)+"px;height:"+(att.h?att.h:200)+"px' data-argi='"+arg.i+"' onclick='ubbcode.autoplayVideo(event,this)' onplay='ubbcode.autoplayVideo(event,this)' onended='ubbcode.autoplayVideo(event,this)' onpause='ubbcode.autoplayVideo(event,this)'></video><img src='about:blank' style='display:none' onerror='ubbcode.videoAdjSize(this.previousSibling)'/>"
			if(window.__APPEMBED)
				self.insertAllImg(arg.i,att.url,tmb,'video',(att.thumb&128) ? (att.url+'.org.jpg'):'')
			...
			return "<div><img src='about:blank' style='width:200px;height:200px' title='"+att.url+"'/></div>"
			}
		else{
			return "<div><a href='"+u+"' target='_blank' class='block_txt block_txt_c40'>"+u.replace(/.+?(\/[^\/]+$)/, '$1')+"</a></div>"
			}
		}
	...
	if(x=u.match(/^\/\/v\.youku\.com\/v_show\/id_(.+?)\.html/i))
		...
		uu = u,	u='//player.bilibili.com/player.html?'+(x[1]=='a' ? 'aid' : 'bvid')+'='+x[2]+'&high_quality=1&page='+((tmp=uu.match(/[\?|&]p=(\d+)/))?tmp[1]:1), w=892, h=499 ,i=1 , thisopt|=512;
	else if(x=u.match(/^\/\/b23\.tv\/([a-zA-Z0-9]+)/i))
		uu = u,	u=u, w=892, h=499 ,i=1 , thisopt|=512;
	...
	else if(x=u.match(/^\/\/(?:www\.)?agn-oediv\.com\/embed\/([a-zA-Z0-9]+)\.html/i))
		uu = '//www.agn-oediv.com/embed/'+x[1]+'.html', u = uu, w=892, h=499, i=1, thisopt|=512
	...
	var br = (__COLOR.docb|0)*2

	if(w+br>arg.maxWidth)
		h=(arg.maxWidth-br)*(h/w),w=arg.maxWidth-br
	...
	if(i){
		if(u.match(/b23\.tv/))
			self.manualLoadCache[id]= "<iframe height="+h+" width="+w+" src='about:blank' frameborder=0 allowfullscreen onload='ubbcode.parseVideoB32(this)' data-src='"+po+u+"'></iframe>"
		else
	...
	var x = "<div class='video' style='display:none;width:"+w+"px'><div class='spacer' style='width:"+w+"px;height:"+h+"px;'><a href='"+(uu?p0+uu:p0+u)+"' style='display:block;height:100%;background:url("+bg+") no-repeat 50% 50%' onclick='ubbcode.playVideo(event,this.parentNode,\""+id+"\","+(thisopt&512)+")' target='_blank'></a></div><div class='moreinfo'>"
	
	if(uu)
		x+= "<a href='"+p0+uu+"' class='xtxt silver' target='_blank'>"+p0+uu+"</a>\x07"

	x+="<i class='silver'>如在以上内容中出现任何广告性信息并不代表本站支持其立场"

	if((thisopt&512) ==0)
		x+= " (播放视频需版主"+(__GP.admincheck ? ' <a href="javascript:void(0)" class="b" style="color:'+__COLOR.gbg6+'" onclick="commonui.atItemBuyUse(event,'+arg.tId+','+(arg.pId|0)+')">设置</a>)':'设置')
	
	x+= "</i></div><img src='about:blank' style='display:none' onerror='ubbcode.videoAdjSize(this.parentNode)'/></div>"
...
ubbcode.videoAdjSize = function(vo){
if(vo.nodeName=='DIV')
	var ow = vo,oh=vo.firstChild
else
	var ow = vo,oh=vo
var w = this.getAvilWidth(vo)
,cw = parseInt(ow.style.width,10)
,ch = parseInt(oh.style.height,10)

if(w < cw){
	w = ( w - this.getStyleInt(vo,'border-left') - this.getStyleInt(vo,'border-right') - 2 ) | 0
	oh.style.width = vo.style.width = w+'px'
	oh.style.height = ch/cw*w+'px'

	}

vo.style.display=''

if(vo.getAttribute('data-playlazy'))
	__INSECTOB.add(vo,function(){
		if(!vo._pl)
			vo.play()
		})
}//

ubbcode.mediaBlk = function(arg,uu){//nouse
var x=uu.pathname.match(/\.(.{1,5})$/)
if(x){
	if((x[1]=='mp4'||x[1]=='ogg'||x[1]=='webm') && (arg.opt&1)){
		return "<a class='video' href='"+uu.url+"' target='_blank' style='text-align:center;display:table-cell' onclick='commonui.cancelEvent(event);if(this.__clicked)return;this.__clicked=1;this.childNodes[0].style.display=\"none\";this.childNodes[1].src=this.href;this.childNodes[1].play()'><i class='silver'>点击开始\x07</i><video controls preload='none'><source src='' type='video/"+x[1]+"'></video></a>"
		}
...
ubbcode.playVideo = function(e,o,id,pl){
if(pl){
	o.style.background="none"
	o.innerHTML=this.manualLoadCache[id]
	commonui.cancelBubble(e)
	return commonui.cancelEvent(e)
	}
}//
...
ubbcode.parseVideoB32=function(o){
var x = o.dataset.src
if(!x || !(x = x.match(/b23\.tv\/([a-zA-Z0-9]+)/)))
 return
o.dataset.src='';
__NUKE.doRequest2(
	'f',function(d){
		...
		if(e)
			return alert(e)
		var bv = d.data ? d.data[0] : 504
		if(bv==404)
			o.contentWindow.document.body.innerHTML = '未找到视频'
		else if(bv==504)
			o.contentWindow.document.body.innerHTML = '服务器忙 请稍后重试'
		else
			o.src='//player.bilibili.com/player.html?bvid='+bv+'&high_quality=1&page=1'
		},
	'u','/nuke.php?__lib=url_conv&__act=bili_from_btv&__output=3', 
	'id', x[1],
	'__output' , 3
	)
}//
...
ubbcode.parseTemplate = function(arg){

if(arg.txt.indexOf('[template')==-1)
	return

arg.txt = arg.txt.replace(/\[template=(.+?)\]([^\x00]+?)\[\/template\]/,function(all,type,content){
	type = type.match(/chartqa/g)
	if(!type)
		return all
	if(type[0]=='chartqa'){
		var qq,aa, ary=[],ml=0;
		(content+'[question]nouse[/question]')
		.replace(/\x07/g,'\n')
		.replace(/\[(question|answer)\]([^\x00]*?)\[\/\1\]/g,function(al,key,txt){
			if((key=='question' && qq)){
				if(!aa)
					aa='N/A'
				ary.push({'title':qq,'name':aa})
				qq = aa = ''
				}
		...
		var ii = Math.ceil(Math.sqrt(ary.length))
...
/*
		...
		var tbl=[],tr = '',tr1='',jj=0,ft=__SETTING.fontSize,bd=(ft/6)|0,fy=ubbcode.parseTemplate.currentFont
		if(!fy)
			fy = ubbcode.parseTemplate.currentFont =commonui.getStyle(document.body,'font-family')
		if(ft<16)ft=16
		
		ml+=2

		for(var i=0;i<ary.length;i++){
			tr+="<td style='border:"+bd+"px solid "+__COLOR.border1+";width:"+(ml*ft)+"px;height:"+(ml*ft)+"px;text-align:center;vertical-align:middle;padding:0'>"+ary[i].name+"</td>"
			tr1+="<td style='border:"+bd+"px solid "+__COLOR.border1+";width:"+(ml*ft)+"px;height:"+(2*ft)+"px;text-align:center;vertical-align:middle;padding:0'>"+ary[i].title+"</td>"
			jj++
			if(jj==ii){
				tbl+='<tr>'+tr+'</tr><tr>'+tr1+'</tr>'
				tr = tr1 = ''
				jj=0
				}
			}
		...
		var aw = (ii*(ml*ft+2*bd)+2*bd+20*bd),ah = (Math.ceil(ary.length/ii)*((ml*ft)+(2*ft)+(4*bd))+(2*bd)+20*bd)
		,cav = document.createElement('canvas')
		,tig = new Image()
		,gdi = ('chartqa'+Math.random()).replace('.','')
		cav.width = tig.width = aw;
		cav.height = tig.height = ah;
		tig.onload = function () {
			cav.getContext('2d').drawImage(this, 0, 0, aw, ah)
			var gi = document.getElementById(gdi)
			if(gi){
				gi.src = cav.toDataURL()
				gi.style.display=''
				}
			setTimeout(function(){cav = null;tig=null})
			}

		tig.src= "data:image/svg+xml;,"+encodeURIComponent("<svg xmlns='http://www.w3.org/2000/svg' style='background-color:"+__COLOR.bg0+"'><foreignObject width='"+aw+"' height='"+ah+"'><body xmlns='http://www.w3.org/1999/xhtml' style='margin:0;font-size:"+ft+"px;font-family:"+fy+"'><table cellspacing='0' style='margin:"+(bd*10)+"px;border:"+bd+"px solid "+__COLOR.border1+";font-weight:bold'>"+tbl+"</table></body></foreignObject></svg>")

		return "<img src='aboutblank' id='"+gdi+"' style='' onerror=''/>"
		}
...
open:function (e,id){
if(window.__APPEMBED){
	var qq = this.cache[id].album, pq=[]
	for(var i=0;i<qq.length;i++){
		if(commonui.ifUrlAttach(qq[i])){
			var a = ubbcode.attach.parseName(qq[i]), q={}
			if(a.type=='img'){
				var o = ubbcode.attach.stripThumb(a.url)
				q.init = (a.thumb & 64) ? (o+'.medium.jpg') : o
				if(a.thumb & 64)
					q.org = o
				pq.push(q)
				}
			...
			if(qq[i].match(/\.(jpg|jpeg|gif|png|webp)$/))
				pq.push({
					'init': qq[i]
					,'org': null
					})
...
if(arg.txt.indexOf('[headline')==-1)
	return
var x=[],self = this
arg.txt = arg.txt.replace(/\x07*\s*\[headline\](.+?)\[\/headline\]\s*\x07*/ig,function ($0,$1){x.push($1);if(x.length==1)return '[headlinehere]';else return ''});
...
if(x){
	...
	for (var k=0;k<x.length;k++)
		{
		var tmp = {txt:x[k]+'',opt:arg.opt}
		ubbcode.ubi.parse(tmp)
		x[k] = tmp.txt
...
for (var k=0;k<this.loadHeadLineElm[id].length;k++)
	...
	if (x.i){
		...
		tmp =  _$('/td','style','padding:0.15em;width:'+w,'class',cls,'innerHTML',x.u)
		tmp.insertBefore(__TXT.svg('label','fill:'+__COLOR.postUiElm_l),tmp.firstChild)
		ltr._.add( tmp )
		j++
		}
	}
...
if(i){
	var xx = _$('/div','class','headlinemask')
	var x  = _$('/div','class','headlineimg',
		xx
		)
	td2._.add(
			_$('/div',
...
o.appendChild(_$('/table','class','headline')._.add(_$('/tbody')._.add(tr)))
}//fe
...
//=============================
ubbcode.writelink=function(u,n,argsId)//#链接生成
{
...
if (v.check<2)
	h = " onclick='ubbcode.showUrlAlert(event,this);commonui.cancelEvent(event);commonui.cancelBubble(event);return false' "+h;
if(v.check==-2 && n)
	n = '被禁止的链接'
...
var r= "<span class='apd' style='vertical-align:0.05em;padding:0 0.15em;color:"+v.color+"'>[</span>"+v.alertHTML+"<a href='"+v.url+"' class='urlincontent' target='_blank' "+h+(n?n:v.url)+"</a><span class='apd' style='vertical-align:0.05em;padding:0 0.15em;color:"+v.color+"'>]</span>"

if(argsId 
	&& v.url.match(/bilibili|topgamer|b23\.tv/) 
	&& (
			v.url.match(/^https?:\/\/www\.bilibili\.com\/video\/(a|b)v([a-z0-9]+)/i)
			|| v.url.match(/^https?:\/\/video\.topgamers\.cn\/video_[a-z0-9_]+\.mp4/i)
			|| v.url.match(/^https?:\/\/b23\.tv\/[a-zA-Z0-9_]+/i)
			)
	)
...
ubbcode.showUrlAlert = function(e,o){
var $=_$,x = this.showUrlAlert.x
if(!x){
	var x = {
		'm':$('/div','style','position:fixed;top:0;left:0;right:0;bottom:0;display:none;opacity:0.95;background:'+__COLOR.bg6,'onclick',function(){ubbcode.showUrlAlert.x.x()}),
		'w':commonui.createCommmonWindow(1,{'onclose':function(){ubbcode.showUrlAlert.x.m.style.display='none'}}),
		'x':function(){commonui.triggerEvent(ubbcode.showUrlAlert.x.w._.__x,'click')}
		}
	document.body.appendChild(x.m)
	document.body.appendChild(x.w)
	this.showUrlAlert.x = x
	}
x.w._.addContent(null)
x.w._.addTitle('即将离开'+location.host.toUpperCase())
x.w._.addContent(
	$('/span','style','font-size:1.08333em')._.add(
		$('/div','style','max-width:35em;border-bottom:0.08em solid '+__COLOR.border2)._.add(o.href),
		$('/div','style','text-align:center;line-height:3em;font-size:1.0769em;font-weight:bold;')._.add(
			'此网址不属于本网站，请谨慎访问',
			$('/br'),
			$('/a','class','block_txt block_txt_c0','style','line-height:1.833em',
				'href',o.href,'target','_blank','onclick',function(){ubbcode.showUrlAlert.x.x()})._.add('继续访问'),
			$('/a','class','block_txt block_txt_c1','style','line-height:1.833em',
				'href','javascript:void(0)','style','margin-left:2em','onclick',function(){ubbcode.showUrlAlert.x.x()})._.add('取消')
			)
		)
	)
x.m.style.display=''
x.w._.show(e,null,64)
...
if(o.style.display!='block'){
	if(!o.style.marginTop || o.style.marginTop=='auto'){
		...
		if(z>0)
			s.marginLeft='-'+z+'px'
		s.marginTop = '-'+(y.height+y.top-x.top)+'px'
...
ubbcode.attach_org_name=function(o,n){}//fe

ubbcode.attach = {
w:window,
parent:ubbcode,
ucache:{},
patt:/\/([a-zA-Z0-9_\-]+)\.([a-zA-Z0-9_\-]+(?:\.mp4)?)(?:\.(thumb|thumb_s|thumb_ss|medium|t)\.[a-zA-Z0-9]+)?$/,
pdata:/^-?[a-z0-9]+Q[a-z0-9\-]+([KZXM][a-z0-9]+)?(?:T([a-z0-9]+))?(?:S([a-z0-9]+)-([a-z0-9]+))?/,
...
stripThumb:function(s){
return s.replace(/\.(thumb|thumb_s|thumb_ss|medium)\.(jpg|jpeg)$/,'')
},//
...
var j = ext.match(/(gif|jpg|jpeg|png|bmp|mp4|mp3|ogg|aac|webm|webp)$/)
if(j){
	//n[2] = j[0]
	switch(j[0]){
		case 'jpg':
...
ifUseThumb:function(noimg,opt,att,i){//arg.noImg, arg.opt, parsed attach, current post img count 
var d = ''
if(noimg & 2)//强制按钮
	d = true
	...
	if(noimg & 1){//自动最小缩略图
		if(att.isThumb){
			...
			if(noimg & 1048576){
				if(att.size>60 || att.size==0)
					d = (att.thumb&64) ? ".medium.jpg" : true
				}
			else{ //if(noimg & 524288){
				if(att.size>25 || att.size==0)
					d = (att.thumb&32) ? ".thumb.jpg" : true//(size>50 ? true : null)
				}
			//d = att.size>60 ? true : ''//biger than 5k
			}
		}
	else if(att.size>100 || att.size==0){//biger than 100k
		if(i>50 || (noimg&4)){
			if(att.thumb&64)
				d = ".medium.jpg"
			else if(att.thumb&32)
				d = ".thumb.jpg"
			else
				d = true
			}
...
return d//返回sf 返回true用按钮 返回空用原图
},
...
if(s && o.previousSibling.nodeName=='H4')
	o.parentNode.removeChild(o.previousSibling)

if(__SETTING.fontSizeOffset)
	o.className += ' fontSizeNoOffset'

o.innerHTML+=x
if(b)
	o.innerHTML+='<div class="clear"></div><a href="javascript:void(0)" class="contentFullWidthButton block_txt block_txt_c40" style="text-align:center;line-height:1.8em;'+(s?'display:block;margin-top:1em':'display:inline-block;padding:0 1em;')+'" onclick="this.style.display=\'none\';ubbcode.attach.disp(\''+id+'\',1)">显示全部附件</a>'
},//fe
...
var orglink='',main='',del='',on=''

if (w.__GP.admincheck ||  w.__CURRENT_UID ==authorId)
	del = (w.__GP.admincheck ? "<a href='javascript:void(0)' onclick=\"if(confirm('是否要把这个附件移到首位'))__NUKE.doRequest2('u','/nuke.php?__lib=del_attach&__act=del_attach&opt=1&__output=3','tid',"+(tid|0)+",'pid',"+(pid|0)+",'aid','"+(aid?aid:name)+"')\" class='block_txt block_txt_c41' title='移到首位'>&#8678;</a> " : '' )
	+"<a href='javascript:void(0)' onclick=\"if(confirm('是否要删除'))__NUKE.doRequest2('u','/nuke.php?__lib=del_attach&__act=del_attach&__output=3','tid',"+(tid|0)+",'pid',"+(pid|0)+",'aid','"+(aid?aid:name)+"')\" class='block_txt block_txt_c41' title='删除'>&#10005</a>";
...
if((type=='img' || type=='video') && thumb){
	...
	if (s[2])
		...
		main = "<a href='"+c.getAttachBase(url)+"/"+url+"' "+st+" target='_blank' title='点击查看原图'><img src='"+c.getAttachBase(url)+"/"+url+"."+s[2]+".jpg' alt='' style='vertical-align:middle;width:"+s[0]+"px'/></a>";
	else
		main = "<span "+st+"><img src='"+c.getAttachBase(url)+"/"+url+"' alt='' style='vertical-align:middle;width:"+s[0]+"px'/></span>";
	return "<div class='left quote' style='margin:0.6em'>"+dscp+'&nbsp;'+del+"<div class='clear'></div>"+main+"<div class='clear'></div></div>";
	}
else{
	/*try{on='?filename='+decodeURIComponent(url_utf8_org_name)}
	catch(x){on=''}*/
	return "<div class='left quote' style='margin:0.6em'>"+dscp+(dscp?'\x07':'')+'&nbsp;'+del+"&nbsp;<a href='"+c.getAttachBase(url)+"/"+url+on+"' target='_blank' class='green'>"+(on ? ubbcode.secureText(on) : name)+"</a> ("+size+" K)</div>";
...
thumbPx:function(tb){//most large thumb
if(tb&64)
	return ".medium.jpg"
else if(tb&32)
	return ".thumb.jpg"
