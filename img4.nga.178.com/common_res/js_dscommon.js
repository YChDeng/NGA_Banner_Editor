...
if(window.__APPEMBED)
	this.loadGroup('app')
else if(ua.match(/iphone|mobile|IEMobile/i))
	this.loadGroup('mobi')
else{
...
for(var k in this){
	if(typeof k =='number' ||k.match(/^bbs_ads\d+$/)){
		if(this[k].onreset && this[k].onreset()){}
		else
			delete this[k]
		}
	}
...
if(a.onclick)
	olo += " onclick='ngaAds[\""+a.id+"\"].onclick.apply(this)' "
	
var bd = window.__APPEMBED ? ('border:1px solid '+__COLOR.postUiElm_ll) : ('border:1px solid '+__COLOR.gbg8)

if(tp){//普通图片广告
	img = "<img src='"+a.file+"' title='"+a.title+"' style='width:"+a.width+";height:"+a.height+";margin:auto;"+ra+"' "+olo+"/>";
	img = (a.url?"<a href='"+a.url+"' target='_blank' ":"<div ")+" style='display:block;"+bd+";width:"+ww+";height:"+hh+";margin:auto;text-align:center;overflow:hidden;"+(a.style?a.style:'')+"' title='"+a.title+"'>"+img+ic+(a.url?"</a>":"</div>")
	return img;
	}
else{//嵌入页面广告等
	var rr=''
	...
	if(a.sgid){
		...
		return "<div style='display:block;"+bd+";width:"+ww+";height:"+hh+";margin:auto;text-align:center;overflow:hidden;'  "+rr+"><div id='"+a.placeholderid+"' style='"+(a.type==5 ? "width:"+ww+";height:"+hh+";" : "width:"+a.width+";height:"+a.height+";"+ra)+"'></div><img src='about:blank' style='display:none' onerror='SG_GG("+a.sgid+")'/></div>"
		}
	if(a.spacerOnly & 1){
		olo = olo.replace('onload=','onerror=').replace('apply(this)','apply(this.parentNode)')
		return "<div style='display:block;overflow:scroll;-webkit-overflow-scrolling: touch;width:"+ww+";height:"+hh+";margin:auto;text-align:center;overflow:hidden;' id='"+a.oid+"'><img src='about:blank' style='display:none' "+(olo?"onerror='ngaAds[\""+a.id+"\"].onload.call(this,this.parentNode)'":'')+"/></div>"
		}
/*
	if (a.type==5 || a.type==1 ){//广点通 baidu
		a.placeholderid = '_ds'+Math.floor(Math.random()*10000000)
		return "<div style='display:block;"+bd+";width:"+ww+";height:"+hh+";margin:auto;text-align:center;overflow:hidden;'  "+rr+"><div id='"+a.placeholderid+"' style='"+(a.type==5 ? "width:"+ww+";height:"+hh+";" : "width:"+a.width+";height:"+a.height+";"+ra)+"'></div><img src='about:blank' style='display:none' "+olo.replace(/^\s*onload=/,'onerror=').replace(/this/g,'this.parentNode')+"/></div>"
		}*/
	...
	img = "<div style='display:block;"+(ic?bd:'border:none')+";width:"+ww+";height:"+hh+";margin:auto;text-align:center;overflow:hidden;"+(a.style?a.style:'')+"' title='"+a.title+"'>"+img+ic+"</div>"
	return img;
	}
