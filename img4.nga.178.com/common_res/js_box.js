...
var P = {}
,urlToAry = function(u){
if(u = u.match(/^([a-z0-9]+:)?(?:\/\/)?([^\x00-\x20\/><"']*)([^\x00-\x1f\?><"']*)([^\x00-\x1f#><"']*)([^\x00-\x1f><"']*)$/i)){
		u = {protocol:u[1]?u[1]:location.protocol
			,host:(u[2]?unescape(u[2]):location.host)
			,pathname:u[3]?u[3]:'/'
			,search:u[4]?u[4]:''
			,hash:u[5]?u[5]:''}
...
,appembedLink = function(h,n){
	var m = urlToAry(h.href)
	if(!m || (m.protocol!='http:' && m.protocol!='https:' )){
		//if(h.href.match(/^javascript:/)){}
		return
		}
	if(m.host.match(/^(bbs\.nga\.cn|nga\.178\.com|ngabbs\.com|bbs\.ngacn\.cc)/)){// / or self domain
		if(m.pathname=='/thread.php'){
			var ri = {'_fromGlobal':1}
			m.search.replace(/(fid|stid)=(-?\d+)/g,function(a,b,c){
				ri[b] = c|0
				})
		...
		else if(m.pathname=='/read.php'){
			if(!h._useloadread){
				var ri = {'_fromGlobal':1}
				m.search.replace(/(tid|topid|pid|#pid|page|opt|authorid|to)=?(\d+)/g,function(a,b,c){
					if(b=='topid')b='pid'
					ri[b]=c|0
					})
				if(ri['#pid'] && !ri.pid){
		...
		else if(m.pathname=='/nuke.php'){
			var ri = {}
			m.search.replace(/(__lib|__act|func|uid|to|username)=([^&#]+)/g,function(a,b,c){
				ri[b]=c
				})
		...
		else if(m.pathname=='/post.php'){

			}
		else if(m.pathname=='/' || !m.pathname){
		...
		if(m.host== __ATTACH_BASE_VIEW_SEC && h.href.match(pregattach)){// attach url
			var na
		...
		__doAction.appDoSync('openLink', {'url':h.href, 'opt':2 ,'_fromGlobal':1})
		return 1
		}
	};//
		...
		if(e.metaKey && (__GP.lesser||__GP.staff) && (e.ctrlKey || e.altKey)){
			...
			if(m.ifapi('nuke.php','read.php')){
				...
				if((tmp.__lib=='ucp'||tmp.func=='ucp') && tmp.uid){
					...
					//else
						commonui.fastPostLoader(tmp.uid,1
							,((e.ctrlKey&&e.altKey) ? 12 : (e.altKey ? 1 : 0))
							)
					return cancelClick(e)
					}
				if(tmp.tid || tmp.pid){
					if(e.altKey)
						commonui.postColector(e,tmp.tid,tmp.pid)
					else
